...
int main(int argc, char *argv[])
{
  ...
  int nCellsX = (int)ceil(Lx/(kappa*dx));
  int nCellsY = (int)ceil(Ly/(kappa*dy));
  // show kappa*dx
  printf("Lx = %lf, Ly = %lf\n",Lx,Ly);
  printf("kappa*dx = %lf, kappa*dy = %lf\n", kappa*dx,kappa*dy);
  ...
  while( t<=tTotal )
    {
      // Update the cell for each particle
      for( i=0; i<nFluid; i++ )
      {
        part[i].cell[X] = (int)ceil(nCellsX*part[i].pos[X]/Lx);
        part[i].cell[Y] = (int)ceil(nCellsY*part[i].pos[Y]/Ly);
      }
  ...
  for( j=0; j<ny; j++)
    {
      for( i=0; i<nx; i++)
          ...
          part[counter].cell[X] = (int)ceil(nCellsX*part[counter].pos[X]/Lx);
          part[counter].cell[Y] = (int)ceil(nCellsY*part[counter].pos[Y]/Ly);
	    counter++;
	    }
    }
